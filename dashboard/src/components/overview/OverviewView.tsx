...
export const OverviewView = component$(() => {
  ...
  const rosterMode = useSignal<'grid' | 'table'>('grid');
  const filterStatus = useSignal<'all' | 'action' | 'star'>('all');
  const showSimulator = useSignal(false);

  const agents = state.agents;
  const fleetKPIs = state.fleetKPIs;
  const workforceKPIs = state.workforceKPIs;
  const simulatedWorkforce = state.simulatedWorkforce;
  const sankeyData = state.sankeyData;

  const filteredAgents = agents.filter((agent: any) => {
    if (filterStatus.value === 'all') return true;
    if (filterStatus.value === 'action')
      return agent.status === 'degraded' || agent.workforce?.performanceGrade === 'C-';
    if (filterStatus.value === 'star')
      return (
        agent.workforce?.performanceGrade === 'A+' || agent.workforce?.performanceGrade === 'A'
      );
    return true;
  });

  const tokenBreakdownData = agents.map((a: any) => ({
    name: a.name.split(' ')[0],
    fullName: a.name,
  ...
  const errorRateData = agents.map((a: any) => ({
    name: a.name.split(' ')[0],
    fullName: a.name,
    agentId: a.id,
  ...
  return (
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group border-emerald-500/20 shadow-xl backdrop-blur-md">
          ...
          <div class="mt-3 pt-3 border-t border-slate-800/80 text-[11px] text-slate-400 flex items-center justify-between">
            ...
            <span class="font-mono text-slate-300 font-bold">
              ${Number(workforceKPIs?.totalComputeCost || 400.37).toFixed(2)} total
            </span>
          </div>
        </div>

        {/* Pillar 2: Human Labor Liberated */}
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group border-sky-500/20 shadow-xl backdrop-blur-md">
          ...
          <div class="mt-3 pt-3 border-t border-slate-800/80 text-[11px] text-slate-400 flex items-center justify-between">
            ...
            <span class="font-mono text-slate-300 font-bold">
              {(workforceKPIs?.totalTasksCompleted || 99400).toLocaleString()} tasks
            </span>
          </div>
        </div>

        {/* Pillar 3: Work Quality & Accuracy */}
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group border-indigo-500/20 shadow-xl backdrop-blur-md">
          ...
          <div class="mt-3 pt-3 border-t border-slate-800/80 text-[11px] text-slate-400 flex items-center justify-between">
            ...
            <span class="font-mono text-amber-300 font-bold">
              {workforceKPIs?.fleetEscalationRate || 4.2}% fleet avg
            </span>
          </div>
        </div>

        {/* Pillar 4: Unit Economics & Response Velocity */}
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 relative overflow-hidden group border-amber-500/20 shadow-xl backdrop-blur-md">
          ...
          <div class="mt-3 pt-3 border-t border-slate-800/80 text-[11px] text-slate-400 flex items-center justify-between">
            ...
            <span class="font-mono text-slate-400 line-through">
              ${workforceKPIs?.avgHumanCostPerWorkUnit || '26.80'} human
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gradient-to-r from-emerald-950/40 via-slate-900/80 to-slate-950/90 border border-emerald-500/30 rounded-2xl p-4 shadow-xl transition-all">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          >
            <span>{showSimulator.value ? 'Close Simulator' : 'Adjust Wage & Scale Sliders'}</span>
            {showSimulator.value ? <ChevronUp class="w-3.5 h-3.5" /> : <ChevronDown class="w-3.5 h-3.5" />}
          </button>
        </div>
          ...
          <div class="mt-4 pt-4 border-t border-slate-800 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="bg-slate-950/60 p-4 rounded-xl border border-slate-800 space-y-2">
              <div class="flex items-center justify-between text-xs font-semibold">
                ...
                <span class="font-mono text-emerald-400 text-sm font-bold">${state.humanHourlyWage}/hr</span>
              </div>
              <input
                type="range"
                min="30"
            ...
            <div class="bg-slate-950/60 p-4 rounded-xl border border-slate-800 space-y-2">
              <div class="flex items-center justify-between text-xs font-semibold">
                ...
                <span class="font-mono text-sky-400 text-sm font-bold">{state.simScaleMultiplier}x</span>
              </div>
              <input
                type="range"
                min="0.5"
          ...
          <div>
            <div class="flex items-center gap-2">
              <h3 class="text-base font-bold text-white tracking-tight flex items-center gap-2">
                <Briefcase class="w-5 h-5 text-sky-400" />
                360° Digital Workforce Roster &amp; Operations Command
              </h3>
              <span class="px-2 py-0.5 text-[10px] font-bold bg-sky-500/10 text-sky-400 border border-sky-500/20 rounded-full">
        ...
        {rosterMode.value === 'grid' ? (
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredAgents.map((agent: any) => {
              const unenforcedRules = agent.rules.filter((r: any) => !r.enforced);
              const activeSkillsCount = agent.skills.filter((s: any) => s.enabled).length;
              const isDegraded = agent.status === 'degraded' || agent.workforce?.performanceGrade === 'C-';

              return (
                <div
                  key={agent.id}
                  onClick$={() => goToAgent(agent.id, isDegraded ? 'refine' : 'workforce')}
                  class={`bg-slate-950/60 border rounded-2xl p-4 transition-all duration-200 cursor-pointer group hover:border-slate-700 hover:shadow-xl hover:shadow-sky-500/5 relative flex flex-col justify-between ${
                    isDegraded
                      ? 'border-rose-500/40 bg-gradient-to-b from-rose-950/20 to-slate-950/80'
                      : 'border-slate-800/80'
                  }`}
                >
                  <div>
                    ...
                    <div class="flex items-start justify-between gap-3 mb-3">
                      <div class="flex items-center gap-2.5">
                        ...
                        <div>
                          <h4 class="text-xs font-bold text-white group-hover:text-sky-300 transition-colors line-clamp-1">
                      ...
                      {agent.status === 'active' ? (
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shrink-0">
                          Healthy
                        </span>
                      ) : (
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/20 shrink-0 animate-pulse">
                          Under Review
                        </span>
                      )}
                    </div>
                    ...
                    {/* Model & Latency */}
                    <div class="flex items-center justify-between text-[11px] bg-slate-900/60 rounded-lg px-2.5 py-1.5 mb-2.5 border border-slate-800/60">
                    ...
                    {agent.workforce && (
                      <div class="flex items-center justify-between text-[10px] bg-emerald-950/30 border border-emerald-500/20 rounded-lg px-2.5 py-1.5 mb-3">
                        <div class="flex items-center gap-1.5">
                          ...
                          <span
                            class={`font-mono font-bold px-1.5 py-0.5 rounded text-[10px] ${
                        ...
                        <span class="font-mono text-emerald-400 font-bold">
                          ${((agent.workforce.totalEconomicValue || 0) / 1000).toFixed(0)}k Value (
                          {Math.round(agent.workforce.netROI).toLocaleString()}x ROI)
                        </span>
                      </div>
                    )}
                    ...
                    <div class="flex items-center justify-between text-[10px] text-slate-400 pt-2 border-t border-slate-800/60">
                  ...
                  <div class="mt-3 pt-2.5 border-t border-slate-800/60 flex items-center justify-between text-[11px]">
                    <span class="text-sky-400 font-semibold group-hover:underline flex items-center gap-1">
                      {isDegraded ? 'Coach & Refine Agent' : 'Open Agent Studio'}{' '}
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
              <thead class="border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                <tr>
                  <th class="pb-3 pl-2">Autonomous Agent</th>
                  <th class="pb-3 text-center">Scorecard</th>
                  <th class="pb-3 text-right">Human Labor Saved</th>
                  <th class="pb-3 text-right">1st-Time Right</th>
                  <th class="pb-3 text-right">Speedup</th>
                  <th class="pb-3 text-right">Unit Cost</th>
                  <th class="pb-3 text-right">Net Value</th>
                  <th class="pb-3 text-center pr-2">Action</th>
                </tr>
              </thead>
                ...
                {filteredAgents.map((agent: any) => {
                  ...
                  return (
                    <tr
                      key={agent.id}
                      onClick$={() => goToAgent(agent.id, isDegraded ? 'refine' : 'workforce')}
                      class="hover:bg-slate-900/50 cursor-pointer transition-colors group"
                    >
                      <td class="py-3.5 pl-2">
                        <div class="flex items-center gap-2.5">
                          ...
                          <div>
                            <span class="font-bold text-white group-hover:text-sky-300 transition-colors block">
                              {agent.workforce?.employeeTitle || agent.name}
                            </span>
                            <span class="text-[10px] text-slate-400">
                              {agent.model} &bull; {agent.role}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td class="py-3.5 text-right font-mono">
                        <span class="text-slate-200 font-bold">{agent.workforce?.firstTimeRightRate}%</span>
                        ...
                        <button
                          ...
                          class={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition-all mx-auto ${
                            isDegraded
                              ...
                              : 'bg-sky-600/20 hover:bg-sky-600 text-sky-300 hover:text-white border border-sky-500/40'
                          }`}
                        >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-md">
          <div class="flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
            ...
            <span class="text-[11px] font-mono text-slate-400">
              Total: {(fleetKPIs.totalTokens / 1000000).toFixed(1)}M
            </span>
          </div>
          ...
          <div class="h-64 w-full">
            <SvgBarChart
              ...
              height={256}
              isStacked={true}
              tooltipFormatter={(val) => `${val}M tokens`}
            />
          </div>
          ...
          <div class="mt-3 pt-3 border-t border-slate-800 text-[11px] text-slate-400 flex items-center justify-between">
            ...
            <button
              onClick$={$(() => {
                goToAgent('deep-research', 'telemetry');
              })}
        ...
        <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-md">
          ...
          <div class="mt-3 pt-3 border-t border-slate-800 text-xs flex items-center justify-between">
            ...
            <button
              onClick$={$(() => {
                goToAgent('promo-shadow', 'refine');
              })}
              class="px-2.5 py-1 rounded-lg bg-sky-600 hover:bg-sky-500 text-white font-medium text-xs flex items-center gap-1.5 transition-all shadow-xs"
            >
