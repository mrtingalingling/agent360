...
export function WorkforceScorecardView() {
  ...
  const simScaleMultiplier = () => dashboardState.simScaleMultiplier;

  const [selectedAgentDetail, setSelectedAgentDetail] = createSignal(null);

  createEffect(() => {
    const list = agents();
    if (!selectedAgentDetail() && list.length > 1) {
      setSelectedAgentDetail(list[1]);
    }
  });

  const valueVsCostData = createMemo(() => {
    return agents().map(a => {
                      ...
                      <td class="py-3.5 pr-3">
                        <span
                          class={`px-2 py-0.5 rounded text-[11px] font-black font-mono ${
                            isTopStar() ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' :
                            isUnderperforming() ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 animate-pulse' :
              ...
              <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800 mb-5">
                <div class="flex items-center gap-3">
                  <div
                    ...
                    style={{
                      "background-color": `${detail.color}20`,
                ...
                <button
                  onClick={() => goToAgent(detail.id, 'refine')}
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-3">
                  ...
                  <div class="space-y-2.5">
                    <For each={detail.workforce?.competencies || []}>
                      {(comp) => (
                        <div class="space-y-1">
                          <div class="flex items-center justify-between text-xs">
                ...
                <div class="space-y-3">
                  ...
                  <div class="space-y-2">
                    <For each={detail.workforce?.escalationReasons || []}>
                      {(esc) => (
                ...
                <div class="space-y-3">
                  ...
                  <div class="bg-slate-950/80 border border-slate-850 rounded-xl p-3.5 text-xs text-slate-300 leading-relaxed">
                    <p class="italic">
                      "{detail.workforce?.coachingNotes}"
                    </p>
