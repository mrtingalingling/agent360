...
import { Info, CheckCircle2, AlertTriangle, RefreshCw, ShieldAlert } from 'lucide-solid';

export function SankeyDiagram(props) {
  const [hoveredNode, setHoveredNode] = createSignal(null);
  const [hoveredLink, setHoveredLink] = createSignal(null);
  const [mousePos, setMousePos] = createSignal({ x: 0, y: 0 });

  const height = () => props.height || 480;
  const width = () => props.width || 980;

  const sankeyGraph = createMemo(() => {
  ...
  return (
    <div class="relative bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl" onMouseMove={handleMouseMove}>
      ...
      <div class="relative w-full overflow-x-auto overflow-y-hidden pt-3">
        ...
        >
          <defs>
            <For each={sankeyGraph().sankeyLinks}>
              {(link, idx) => {
          ...
          {/* Nodes */}
          <g class="nodes">
            <For each={sankeyGraph().sankeyNodes}>
              {(node, idx) => {
        ...
        <Show when={hoveredLink() !== null && sankeyGraph().sankeyLinks[hoveredLink()]}>
          {(() => {
            const link = sankeyGraph().sankeyLinks[hoveredLink()];
            return (
              <div
                ...
                style={{
                  left: `${Math.min(width() - 240, Math.max(10, mousePos().x + 15))}px`,
                  top: `${Math.max(10, mousePos().y - 20)}px`
                }}
              >
                <div class="flex items-center gap-1.5 text-slate-300 font-semibold mb-1">
                  <span class="w-2 h-2 rounded-full" style={{ "background-color": link.source.color }}></span>
                  <span>{link.source.name}</span>
                  <span class="text-slate-500">→</span>
                  <span class="w-2 h-2 rounded-full" style={{ "background-color": link.target.color }}></span>
