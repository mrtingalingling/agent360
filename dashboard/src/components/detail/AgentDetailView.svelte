<script>
  ...
  import TokenWaterfallTrace from '../telemetry/TokenWaterfallTrace.svelte';
  import AgentIAMControls from '../iam/AgentIAMControls.svelte';
  import {
    Coins,
    Cpu,
    Clock,
  ...
  const agentActiveSubTab = $derived(dashboardState.agentActiveSubTab);
  const recentTraces = $derived(dashboardState.recentTraces);
  const cloudLogs = $derived(dashboardState.cloudLogs);

  let subTab = $state('refine');
  ...
  let newSkillForm = $state({ name: '', description: '', riskLevel: 'low' });

  let showAddRuleModal = $state(false);
  let newRuleForm = $state({ name: '', description: '', category: 'safety' });

  let searchQuery = $state('');
  let directiveApplied = $state(false);

  // Sync draft state whenever selected agent changes
  $effect(() => {
    if (selectedAgent) {
      draftSkills = JSON.parse(JSON.stringify(selectedAgent.skills || []));
      draftRules = JSON.parse(JSON.stringify(selectedAgent.rules || []));
      draftConfig = JSON.parse(JSON.stringify(selectedAgent.fineTuneConfig || {}));
  ...
  $effect(() => {
    if (agentActiveSubTab) {
      subTab = agentActiveSubTab;
    }
  });

  function handleToggleSkill(skillId) {
    draftSkills = draftSkills.map(s => s.id === skillId ? { ...s, enabled: !s.enabled } : s);
  }

  function handleToggleRule(ruleId) {
    draftRules = draftRules.map(r => r.id === ruleId ? { ...r, enforced: !r.enforced } : r);
  }
  ...
  function handleCreateSkill(e) {
    ...
    draftSkills = [...draftSkills, newSkill];
    dashboardState.addAgentSkill(selectedAgent.id, newSkill);
    newSkillForm = { name: '', description: '', riskLevel: 'low' };
  ...
  function handleCreateRule(e) {
    ...
    newRuleForm = { name: '', description: '', category: 'safety' };
  ...
  function handleRunSimulation() {
    isSimulating = true;
    setTimeout(() => {
      const result = dashboardState.runBenchmarkSimulation(selectedAgent.id, draftConfig, draftRules);
  ...
  function handleSaveAllRefinements() {
    dashboardState.refineAgent(selectedAgent.id, {
      skills: draftSkills,
      rules: draftRules,
  ...
  const agentTraces = $derived(
    recentTraces.filter(t => t.agentId === selectedAgent.id)
  );
</script>
...
<div class="space-y-6">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
    {#each agents as agent}
      ...
      <button
        ...
        class="p-3 rounded-xl border text-left transition-all {isSelected ? 'bg-slate-900 border-brand-500 shadow-lg shadow-brand-500/15 ring-1 ring-brand-500' : 'bg-slate-950/60 border-slate-800 hover:border-slate-700 hover:bg-slate-900/40'}"
      >
  ...
  <div class="glass-panel rounded-2xl p-6">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-slate-800">
      <div class="flex items-start gap-4">
        <div
          ...
          style="background-color: {selectedAgent.color}20; border: 1px solid {selectedAgent.color}40; color: {selectedAgent.color};"
        >
          <Cpu class="w-6 h-6" />
        </div>
        <div>
          <div class="flex items-center gap-3 flex-wrap">
            ...
            {:else if selectedAgent.status === 'warning'}
      ...
      <div class="flex items-center gap-4 text-xs flex-wrap">
        {#if selectedAgent.workforce}
          <div class="text-right border-r border-slate-800 pr-4">
            ...
            <span class="font-mono text-emerald-300 font-bold">Grade {selectedAgent.workforce.performanceGrade} ({Math.round(selectedAgent.workforce.netROI).toLocaleString()}x ROI)</span>
          </div>
        {/if}
        <div class="text-right">
          <span class="text-slate-500 text-[10px] block">Runs &amp; Cost</span>
          <span class="font-mono text-slate-200 font-bold">{selectedAgent.totalRuns.toLocaleString()} runs (${selectedAgent.costEstimate})</span>
        </div>
    ...
    <div class="flex items-center justify-between gap-4 pt-4 flex-wrap">
      <div class="flex items-center gap-2 flex-wrap">
        <button
          onclick={() => { subTab = 'refine'; dashboardState.setAgentActiveSubTab('refine'); }}
          class="flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all {subTab === 'refine' ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'}"
        >
        ...
        <button
          onclick={() => { subTab = 'workforce'; dashboardState.setAgentActiveSubTab('workforce'); }}
          class="flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all {subTab === 'workforce' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'}"
        >
        ...
        <button
          onclick={() => { subTab = 'telemetry'; dashboardState.setAgentActiveSubTab('telemetry'); }}
          class="flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all {subTab === 'telemetry' ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'}"
        >
        ...
        <button
          onclick={() => { subTab = 'traces'; dashboardState.setAgentActiveSubTab('traces'); }}
          class="flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all {subTab === 'traces' ? 'bg-brand-600 text-white shadow-sm' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'}"
        >
        ...
        <button
          onclick={() => { subTab = 'iam'; dashboardState.setAgentActiveSubTab('iam'); }}
          class="flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all {subTab === 'iam' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-900/60 text-slate-400 hover:text-slate-200 border border-slate-800'}"
        >
      ...
      <div class="text-[11px] text-slate-400 hidden sm:block">
        Targeting <span class="text-brand-400 font-semibold">{selectedAgent.name}</span>
      </div>
    </div>
  </div>
  ...
  {#if subTab === 'refine'}
    <div class="space-y-6">
      {#if selectedAgent.coachingDirective}
        <div class="bg-gradient-to-r from-rose-950/40 via-amber-950/30 to-slate-900/90 border border-rose-500/40 rounded-2xl p-5 shadow-2xl relative overflow-hidden">
          <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="space-y-1.5">
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-slate-400 font-mono">
                  Issued by: <span class="text-slate-200 font-semibold">{selectedAgent.coachingDirective.supervisor}</span>
                </span>
              </div>
              ...
              <p class="text-xs text-slate-300">
                <span class="text-amber-400 font-semibold">Recommended Intervention:</span> {selectedAgent.coachingDirective.actionPlan}
              </p>
            </div>
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
              <button
                ...
                class="px-4 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 transition-all shadow-lg {directiveApplied ? 'bg-emerald-600/30 text-emerald-300 border border-emerald-500/40 cursor-default' : 'bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white shadow-rose-600/25 active:scale-95'}"
              >
          ...
          {#if directiveApplied}
            <div class="mt-3 pt-3 border-t border-rose-500/20 flex items-center justify-between text-xs text-emerald-300 animate-in fade-in">
        ...
        <div class="lg:col-span-7 space-y-6">
          ...
          <div class="glass-panel rounded-2xl p-5 space-y-4">
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <div class="flex items-center gap-2">
                        ...
                        <span
                          class="text-[9px] px-1.5 py-0.2 rounded font-semibold uppercase {skill.riskLevel === 'high' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : skill.riskLevel === 'medium' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}"
                        >
                          {skill.riskLevel} risk
                        </span>
                        {#if skill.runs !== undefined}
            ...
            {#if showAddSkillModal}
              <form onsubmit={handleCreateSkill} class="bg-slate-950 border border-brand-500/40 rounded-xl p-3.5 space-y-3 animate-in fade-in">
                <div class="flex items-center justify-between text-xs font-bold text-white border-b border-slate-800 pb-2">
                  ...
                  <button type="button" onclick={() => showAddSkillModal = false} class="text-slate-400 hover:text-white">
                    <X class="w-3.5 h-3.5" />
                  </button>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <select
                      id="new-skill-risk"
                      bind:value={newSkillForm.riskLevel}
                ...
                <div>
                  ...
                  <input
                    ...
                    bind:value={newSkillForm.description}
                    class="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100"
                  />
                </div>
                <div class="flex justify-end gap-2 pt-1">
                  <button
                    type="button"
                    onclick={() => showAddSkillModal = false}
          ...
          <div class="glass-panel rounded-2xl p-5 space-y-4">
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <div class="flex items-center gap-2">
                        ...
                        <span
                          class="text-[9px] px-1.5 py-0.2 rounded font-semibold uppercase {rule.category === 'safety' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : rule.category === 'privacy' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'bg-sky-500/10 text-sky-400 border border-sky-500/20'}"
                        >
                          {rule.category}
                        </span>
                      </div>
            ...
            <!-- Add Rule Modal -->
            {#if showAddRuleModal}
              <form onsubmit={handleCreateRule} class="bg-slate-950 border border-emerald-500/40 rounded-xl p-3.5 space-y-3 animate-in fade-in">
                <div class="flex items-center justify-between text-xs font-bold text-white border-b border-slate-800 pb-2">
                  <span>Create Governance Rule</span>
                  <button type="button" onclick={() => showAddRuleModal = false} class="text-slate-400 hover:text-white">
                    <X class="w-3.5 h-3.5" />
                  </button>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      bind:value={newRuleForm.category}
                      class="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100"
                    >
                ...
                <div>
                  ...
                  <input
                    ...
                    bind:value={newRuleForm.description}
                    class="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100"
                  />
                </div>
                <div class="flex justify-end gap-2 pt-1">
          ...
          <div class="glass-panel rounded-2xl p-5 space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <div class="grid grid-cols-2 gap-2">
                {#each [
                  ...
                  <button
                    type="button"
                    onclick={() => draftConfig.model = m.id}
            ...
            <div class="space-y-1.5 pt-2">
              ...
              <div class="grid grid-cols-3 gap-2">
                {#each ['strict', 'balanced', 'permissive'] as gm}
                  <button
                    type="button"
                    onclick={() => draftConfig.groundingMode = gm}
        ...
        <div class="lg:col-span-5 space-y-5">
          <div class="glass-panel rounded-2xl p-5 border-brand-500/30 shadow-xl space-y-4">
            ...
            <div class="pt-2 border-t border-slate-800 space-y-2">
              ...
              >
                <Save class="w-4 h-4" />
                <span>Save &amp; Deploy Refinements to Agent</span>
              </button>
              ...
              >
                ...
                <span>Reset to Stored State</span>
              </button>
            </div>
          </div>
          ...
          <div class="glass-panel rounded-2xl p-4 space-y-2 text-xs text-slate-400">
            <div class="flex items-center gap-2 text-slate-300 font-semibold">
              ...
              <span>Governance &amp; Refinement Summary</span>
            </div>
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="glass-panel rounded-2xl p-5">
          ...
          <div class="space-y-3">
            {#each selectedAgent.errors.hallucinationExamples as item}
        ...
        <div class="glass-panel rounded-2xl p-5">
          ...
          <div class="space-y-3">
            {#each selectedAgent.errors.repromptReasons as item}
              <div class="space-y-1">
                <div class="flex items-center justify-between text-xs">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs">
          ...
          <tbody class="divide-y divide-slate-850">
            {#each agentTraces.filter(t => !searchQuery || t.prompt.toLowerCase().includes(searchQuery.toLowerCase()) || t.id.includes(searchQuery)) as trace}
              <tr class="hover:bg-slate-900/60 transition-colors">
                ...
                <td class="py-2.5">
                  ...
                  {:else if trace.status === 'reprompt_loop'}
      ...
      {#if cloudLogs && cloudLogs.length > 0}
        <div class="mt-6 pt-5 border-t border-slate-800">
          ...
          <div class="bg-slate-950 rounded-xl border border-slate-800 p-3 font-mono text-[11px] max-h-48 overflow-y-auto space-y-1.5">
            {#each cloudLogs as log, idx}
              <div class="flex items-start gap-2 text-slate-300">
                <span class="text-slate-500 shrink-0 select-none">{log.timestamp?.substring(11, 19)}</span>
    ...
    <div class="space-y-6 animate-in fade-in duration-200">
      <div class="glass-panel rounded-2xl p-6 border-emerald-500/30 relative overflow-hidden">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span
                  class="px-2 py-0.5 rounded text-[11px] font-black font-mono {selectedAgent.workforce?.performanceGrade === 'A+' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : selectedAgent.workforce?.performanceGrade === 'C-' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 animate-pulse' : 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40'}"
                >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-5">
          ...
          <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-4">
            <span class="text-slate-400 text-xs font-semibold block mb-1 flex items-center gap-1">
              <Clock class="w-3.5 h-3.5 text-sky-400" /> Human Labor Liberated
            </span>
          ...
          <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-4">
            ...
            <span class="text-[10px] text-slate-500 block mt-1 line-through">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="glass-panel rounded-2xl p-5 space-y-4">
          ...
          <div class="space-y-3">
            {#each (selectedAgent.workforce?.competencies || []) as comp}
              <div class="space-y-1">
                <div class="flex items-center justify-between text-xs">
        ...
        <div class="glass-panel rounded-2xl p-5 space-y-4">
          ...
          <div class="space-y-2.5">
            {#each (selectedAgent.workforce?.escalationReasons || []) as esc}
          ...
          <div class="bg-slate-950/80 border border-slate-800 rounded-xl p-4 text-xs text-slate-300 space-y-3">
            ...
            <div class="pt-3 border-t border-slate-800 text-[11px] space-y-1.5">
              <div class="flex items-center justify-between">
                ...
                <span class="font-mono text-emerald-400 font-bold">{selectedAgent.workforce?.autonomousResolutionRate}%</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-slate-400">Execution Velocity:</span>
                <span class="font-mono text-sky-400 font-bold">{selectedAgent.workforce?.speedupMultiplier}x vs Human</span>
              </div>
            </div>
          </div>
