...
import { createDashboardState } from '../src/state/dashboardState';

describe('Qwik Dashboard State Truth Table Suite', () => {
  let state: ReturnType<typeof createDashboardState>;
  ...
  it('test_coaching_preset_application', () => {
    ...
    const agent = state.agents.find((a: any) => a.id === 'promo-shadow');
    ...
    expect(agent.rules.find((r: any) => r.id === 'rl-ps-1').enabled).toBe(true);
    expect(agent.rules.find((r: any) => r.id === 'rl-ps-2').enabled).toBe(true);
  });
