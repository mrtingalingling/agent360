<script>
  ...
  const cloudWaterfall = $derived(dashboardState.cloudWaterfall);
  let selectedTurnIdx = $state(19);

  const turns = $derived(cloudWaterfall?.turns || []);
  const currentTurn = $derived(turns[selectedTurnIdx] || turns[turns.length - 1] || null);

  const macroStats = $derived.by(() => {
  ...
  const maxLatency = $derived(
    turns.length ? Math.max(...turns.map(t => t.latency.totalLatencyMs), 1600) : 1600
  );
</script>

{#if !turns.length}
  <div class="bg-white rounded-xl border border-slate-200 p-8 text-center shadow-xs">
    <Server class="w-8 h-8 text-slate-400 mx-auto mb-2 animate-pulse" />
    <p class="text-sm font-medium text-slate-700">Connecting to telemetry waterfall stream...</p>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
      ...
      <div class="mt-4 space-y-2 max-h-[420px] overflow-y-auto pr-2">
        {#each turns as turn, idx}
          ...
          <div
            onclick={() => selectedTurnIdx = idx}
            class="group cursor-pointer rounded-lg p-2 transition-all border {isSelected ? 'bg-indigo-50/80 border-indigo-300 ring-2 ring-indigo-500/20' : 'hover:bg-slate-50 border-slate-100 hover:border-slate-200'}"
          >
            <div class="flex items-center justify-between text-xs mb-1.5 font-mono">
              <div class="flex items-center gap-2">
                <span class="px-1.5 py-0.5 rounded text-[10px] font-bold {isSelected ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-700'}">
                  Turn #{turn.stepIndex}
                </span>
            ...
            <!-- Gantt Bar -->
            <div class="w-full bg-slate-100 h-4 rounded-sm overflow-hidden flex items-center relative">
              <div
                style="width: {prepPct}%"
      ...
      <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
        <button
          onclick={() => selectedTurnIdx = Math.max(0, selectedTurnIdx - 1)}
        ...
        <button
          onclick={() => selectedTurnIdx = Math.min(turns.length - 1, selectedTurnIdx + 1)}
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between">
          ...
          <div class="mt-4 p-3 bg-emerald-50 rounded-lg border border-emerald-200 text-xs text-emerald-900 flex items-start gap-2.5">
            ...
            <div>
              <span class="font-bold">Prompt Cache Savings Applied:</span> Reused {currentTurn.tokens.cachedPromptTokens.toLocaleString()} cached tokens, saving <strong class="text-emerald-700">${currentTurn.finops.dollarsSaved}</strong> on this turn alone. Net turn cost: <strong class="font-mono">${currentTurn.finops.netCostUsd}</strong> (vs ${currentTurn.finops.rawCostUsd} raw).
            </div>
          </div>
        </div>
